  ...
  Droppables.add(dropobj, {
   ...
   onDrop: function(dragobj) {
    ...
    if (land_stadt[dropobj.id] == dragobj.id) {
     // ja: Erfolgsmeldung
     $('anleitung').update(dragobj.firstChild.nodeValue + ' Te encuentas en ' + dropobj.firstChild.nodeValue + ' - Obtienes un punto!');
     score(1);
    } else {
     $('anleitung').update('ay de ti ...');
     score(-1);
    }
   },
...
function score(wert) {
 ...
 $('punktestand').update(punkte + ' Punto' + (punkte != 1? 'e' : ''));
 new Effect.Highlight('punktestand', wert > 0? grueneffekt : roteffekt);
 // 4 Punkte: Spiel gewonnen
 if(punkte > 3) {
  $('anleitung').update('Eres demasiado bueno para este juego. ¡Felicitaciones!');
  exit(1);
 // -4 Punkte: Spiel verloren
 } else if(punkte < -3) {
  $('anleitung').update("El lúpulo y la malta están perdidos para ti...");
  exit(0);
 }
}
